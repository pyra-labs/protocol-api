...
export const getDepositLimits = async (address, connection) => {
    const limits = getMarketIndicesRecord(0);
    for (const marketIndex of MarketIndex) {
        limits[marketIndex] = await fetchDepositLimit(connection, address, marketIndex);
    }
    return limits;
};
async function fetchDepositLimit(connection, pubkey, marketIndex) {
...
async function fetchMaxDepositLamports(pubkey, connection, marketIndexSol) {
    ...
    const [deposit, blockhash] = await Promise.all([
        depositPromise,
        connection.getLatestBlockhash().then(res => res.blockhash)
    ]);
    const { ixs: depositIxs } = deposit;
    const [computeUnitLimit, computeUnitPrice, balanceLamports, wSolAtaRent] = await Promise.all([
...
async function fetchMaxDepositSpl(pubkey, connection, mint) {
