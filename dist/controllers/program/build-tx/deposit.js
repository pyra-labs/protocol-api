...
export const buildDepositTransaction = async (address, amountBaseUnits, marketIndex, repayingLoan, useMaxAmount, connection, quartzClient) => {
    let user;
    try {
    ...
    const { ixs, lookupTables } = await makeDepositIxs(connection, address, amountBaseUnits, marketIndex, user, repayingLoan);
    const transaction = await buildTransaction(connection, ixs, address, lookupTables);
    return Buffer.from(transaction.serialize()).toString("base64");
};
export const makeDepositIxs = async (connection, address, amountBaseUnits, marketIndex, user, repayingLoan) => {
    ...
    const { ixs, lookupTables, } = await user.makeDepositIx(amountBaseUnits, marketIndex, repayingLoan);
    return {
