...
export class TxController extends Controller {
    MAX_DURATION = 70; // 70 second maximum in Vercel
    connection;
    constructor() {
        super();
        this.connection = new Connection(config.RPC_URL);
    }
    confirmTx = async (req, res, next) => {
        try {
    ...
    sendTransaction = async (req, res, next) => {
        ...
        try {
            const signature = await retryWithBackoff(async () => this.connection.sendRawTransaction(body.transaction, {
